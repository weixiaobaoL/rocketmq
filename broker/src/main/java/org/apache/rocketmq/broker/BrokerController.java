...
public class BrokerController {
    ...
    public boolean initialize() throws CloneNotSupportedException {
        //region 加载一些磁盘上的数据到内存中，1.Topic的配置、Consumer的消费offset、Consumer的订阅组、过滤器
        ...
        //只有配置信息都加载成功以后才能做具体的一些操作
        if (result) {
            try {
                //region 创建MessageStore，用来管理磁盘上的消息
                this.messageStore =
                    new DefaultMessageStore(this.messageStoreConfig, this.brokerStatsManager, this.messageArrivingListener,
                        this.brokerConfig);
                //endregion

                //region 如果启用了dleger技术进行主从同步以及管理commitlog。这里会初始化一些dleger相关的组件
        ...
        if (result) {
            ...
            //region 初始化一些请求处理相关的线程池
            //region sendMessageExecutor 发送消息过来的时候的处理线程池
            ...
            //region 核心逻辑: 用于给NameServer发送心跳的后台线程池
            ...
            //region 定时进行consumer消费 offset持久化到磁盘的逻辑
            this.scheduledExecutorService.scheduleAtFixedRate(new Runnable() {
                @Override
                public void run() {
                    try {
            ...
            //region 定时对consumer filter过滤器进行持久化的任务
            this.scheduledExecutorService.scheduleAtFixedRate(new Runnable() {
                @Override
                public void run() {
                    try {
            ...
            //FIXME: 问题，在Broker启动的时候如果这里有两种加载NameServer地址的方式，对于没有给定NameServer配置信息的时候，是不是就没法获取最新的NameServer的地址信息。
            // 因为没有对应的线程池处理这个更新操作
            ...
            //region 核心逻辑: 当开启了dleger的时候，会做一些初始化的操作。
            if (!messageStoreConfig.isEnableDLegerCommitLog()) {
